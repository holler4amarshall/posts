...
# JSON Schema describing the structure of a post (for validation of post format)
post_schema = {
    "properties": {
        "title" : {"type" : "string"},
        "body": {"type": "string"}
...
def posts_get():
    ...
    if title_like and body_like: 
        posts = posts.filter(models.Post.title.contains(title_like) & (models.Post.title.contains(body_like)))
    elif title_like:
...
def post_put(id):
    ...
    # Return a 200 Success response, containing the post as JSON and with the
    # Location header set to the location of the post
    data = json.dumps(post.as_dictionary())
    headers = {"Location": url_for("post_get", id=post.id)}
